...
| 플러그인 | 스킬 | 설명 | 출처 |
|---------|------|------|------|
| harness | `/harness` | 경량 하네스 — 에이전트용 지속 지식 문서 (AGENTS.md 지도 + harness/ 의 ARCHITECTURE·ADR·GLOSSARY·LESSONS) 와 선택적 레이어 검사 (hook + CI) 를 단일 진입점으로 관리. 상태 자동 감지로 setup (0→1 구축) / update (버전 동기화 + v1→v2 마이그레이션) / audit (문서 낡음 점검) / reflect (세션 학습 승격) 분기 | |
