...
장수 브랜치(같은 이름으로 "기능 묶음 → PR → 병합(원격 브랜치 삭제) → 다시 시작" 을 반복하는 브랜치)는 재생성이 주기적으로 필요하고, 그 사이사이 진행 중인 브랜치에 최신 main 을 당겨오는 일이 섞인다.
...
- 기본 브랜치는 `origin/HEAD` 기준으로 파악한다 (모르면 `git remote show origin | grep 'HEAD branch'`).
...
- 기본 브랜치에는 pull 만 한다. 직접 푸시하지 않는다 (기본 브랜치 직접 푸시를 금지하는 repo 가 많다).
